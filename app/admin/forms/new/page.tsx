...
import { useMemo, useState } from "react";
 ...
 export default function AdminNewFormPage() {
  ...
  const [fields, setFields] = useState<BuilderField[]>([
    { id: rnd(8), type: "text", label: "Your Name", required: true },
  ]);
   const [error, setError] = useState<string | null>(null);
   const [saving, setSaving] = useState(false);
 
   const addField = (type: FieldType) => {
     setFields((prev) => [...prev, { id: rnd(8), type, label: "", required: false }]);
   };
     ...
     <div className="space-y-8">
       <div className="flex items-center justify-between">
         <div>
           ...
           <p className="mt-2 text-zinc-600">Compose fields and save to your organization.</p>
         </div>
       </div>
       ...
       <div className="grid gap-8 lg:grid-cols-2">
         <div className="space-y-4">
           ...
           <div className="space-y-3">
             <div className="text-sm font-medium text-zinc-700">Add Field</div>
             <div className="flex flex-wrap gap-2">
               <button onClick={() => addField("text")} className="rounded-lg ring-1 ring-zinc-200 px-3 py-1.5 text-sm hover:bg-zinc-50">Text</button>
               <button onClick={() => addField("textarea")} className="rounded-lg ring-1 ring-zinc-200 px-3 py-1.5 text-sm hover:bg-zinc-50">Textarea</button>
               <button onClick={() => addField("number")} className="rounded-lg ring-1 ring-zinc-200 px-3 py-1.5 text-sm hover:bg-zinc-50">Number</button>
               <button onClick={() => addField("select")} className="rounded-lg ring-1 ring-zinc-200 px-3 py-1.5 text-sm hover:bg-zinc-50">Select</button>
               <button onClick={() => addField("date")} className="rounded-lg ring-1 ring-zinc-200 px-3 py-1.5 text-sm hover:bg-zinc-50">Date</button>
             </div>
           </div>
         </div>
         <div className="space-y-4">
